...
int main(int argc, char *argv[])
{
	...
	vtkSmartPointer<vtkPolyData> data = curvaturesFilter->GetOutput();
	vtkSmartPointer<vtkDataArray> curvatures = 
		static_cast<vtkSmartPointer<vtkDataArray>>(data->GetPointData()->GetAttribute(0));
	for (vtkIdType i = 0; i < curvatures->GetNumberOfTuples(); i++)
	{
		cout << i << ":" << curvatures->GetTuple(i)[0] << endl;
		/*cout << curvatures->GetTuple(i)[1] << endl;
		cout << curvatures->GetTuple(i)[2] << endl;*/
	}
